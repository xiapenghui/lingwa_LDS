...
const regionDataComponent = ({ regionData, dispatch }) => {
  const { productList, areaList, shifList } = regionData;
  ...
  const columns = [
...

    {
      ...
      width: 100,
      render: (text, record) => {
        console.log(record.ke);
        let color =
          parseInt(record.ke * 100) < record.targetke ? "red" : "green";
      ...
      <ProTable
        ...
        headerTitle={
          <>
            ...
            >
              *gap=排班-工作时间-借出+借入-休假-领班T4 , *T0=OT-DT-T1-T2-T3
            </span>
          </>
        }
        actionRef={actionRef}
...
export default connect(({ regionData }) => ({ regionData }))(
  regionDataComponent
);
