...
const productGroupComponent = ({
  ...
  const getColumns = () => [
...

    {
      ...
      valueType: 'text',
      align: 'center',
      initialValue: IsUpdate ? UpdateDate.familyname : '',
      formItemProps: {
        rules: [
          {
            required: true,
  ...
  const handleUpdate = async (fields) => {
    ...
    try {
      let data = await updatePut({ familyid: UpdateDate.familyid, ...fields });
      if (data.status == '200') {
        hide();
        message.success(data.message);
        return true;
  ...
  const handleRemove = async (selectedRows) => {
    ...
    try {
      let data = await deleted({
        familyids: selectedRows.map((row) => row.familyid),
      });
  ...
  const downloadExcel = async (selectedRows) => {
    ...
    option.fileName = '工厂信息'
    option.datas = [
      {
        sheetData: dataTable,
        sheetName: 'sheet',
        sheetFilter: ['familyname', 'remark'],
        sheetHeader: ['工厂名称', '备注'],
      }
    ];
...
export default connect(({ productGroup }) => ({ productGroup }))(productGroupComponent);
