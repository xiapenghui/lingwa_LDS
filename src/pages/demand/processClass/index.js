...
import {
  ...
  // addPost,
  // updatePut,
} from "@/services/demand/processClass";
import { BackgroundColor } from "chalk";

const processClassComponent = ({ processClass, dispatch }) => {
  const { productList, areaList, shifList } = processClass;
      ...
      for (let i in dataList) {
        let obj = {
          ...
          OEE:
            dataList[i].OEE === "NaN"
              ? 0 + "%"
              : parseInt(dataList[i].OEE * 100) + "%",
          NEE:
            dataList[i].NEE === "NaN"
              ? 0 + "%"
              : parseInt(dataList[i].NEE * 100) + "%",
          SUR:
            dataList[i].SUR === "NaN"
              ? 0 + "%"
              : parseInt(dataList[i].SUR * 100) + "%",
          targetoee:
            dataList[i].targetoee === "NaN"
              ? 0 + "%"
              : parseInt(dataList[i].targetoee) + "%",
          targetsur:
            dataList[i].targetsur === "NaN"
              ? 0 + "%"
              : parseInt(dataList[i].targetsur) + "%",
          gap: dataList[i].gap,
...
export default connect(({ processClass }) => ({ processClass }))(
