...
const personnelComponent = ({
  ...
  const getColumns = () => [
...

    {
      ...
      initialValue: !IsUpdate ? '' : (UpdateDate.defaultlineid ? UpdateDate.defaultlineid.toString() : ''),
      renderFormItem: (_, { type, defaultRender, ...rest }, form) => {
        if (type === 'form' || type === 'table') {
          // 返回新的组件
          let newList = []
...

    {
      ...
      // initialValue: IsUpdate ? UpdateDate.defaultlineid.toString() : '',
      initialValue: !IsUpdate ? '' : (UpdateDate.defaultshiftclass ? UpdateDate.defaultshiftclass.toString() : ''),
      renderFormItem: (_, { type, defaultRender, ...rest }, form) => {
        if (type === 'form' || type === 'table') {
          // 返回新的组件
          let newList = []
