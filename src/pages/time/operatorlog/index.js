...
const unscheduledComponent = ({ operatorlog, dispatch }) => {
  const {
    departmentList,
    productList,
    personList,
  ...
  const getColumns = () => [
    {
      ...
      initialValue: !IsUpdate
        ? ""
        : UpdateDate.employeeid
        ? UpdateDate.employeeid.toString()
        : "",
  ...
  const query = async (params, sorter, filter) => {
    const TableList = postListInit({
      employeeid:
        Number(params.employeeid) == null ? "" : Number(params.employeeid),
...
export default connect(({ operatorlog }) => ({ operatorlog }))(
